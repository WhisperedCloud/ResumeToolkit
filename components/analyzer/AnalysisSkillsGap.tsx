...
export const AnalysisSkillsGap: React.FC<AnalysisSkillsGapProps> = ({ skills }) => {
    ...
    // FIX: The original implementation of `reduce` had type inference issues which caused `skillList` to be of type `unknown`, leading to an error when calling `.map()`. By explicitly typing the accumulator (`acc`) parameter, we ensure `groupedSkills` is correctly typed, resolving the error.
    const groupedSkills = skills.reduce((acc: Record<string, SkillGap[]>, skill) => {
